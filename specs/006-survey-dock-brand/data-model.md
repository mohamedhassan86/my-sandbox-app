...
_optional_ presentation-copy fields, (b) defines the token delta the brand is built from,
...
attachments, and the submission payload are byte-identical. Invalid _new_ fields produce
...
`.toast-region`. The PrimeNG bridge keeps its selectors; only token _values_ flowing
