...
| `S1Q3` | `dropdown` | false | 4 options | 0 |
| `S2Q1` | `rating` | **true** | `minValue: 1`, `maxValue: 10`, `leftLabel`/`rightLabel` | 0 |
| `S2Q2` | `satisfaction` | false | 5 options | 0 |
| `S2Q3` | `checkbox` | false | 5 options | 0 |
| `S3Q1` | `radio` | **true** | 5 options | 0 |
...
- Required distribution: exactly 3 required questions — the first question of each
  step: `S1Q1` (radio), `S2Q1` (rating), `S3Q1` (radio); all other questions optional,
  and no question defines a min/max selection bound (FR-006, clarified 2026-09-11).
...
|---|---|---|
| `S1Q1`, `S3Q1` | String matching one of the question's option values | N/A — required |
| `S1Q3`, `S3Q3` | String matching one of the question's option values | Absent/`null` |
...
| `S2Q1` | Numeric string `"1"`–`"10"` | N/A — required |
...
- **Response** (`response.validator.ts`): required-empty check for `S1Q1`, `S2Q1`,
  `S3Q1` (one per step); dropdown string-in-options checks for `S1Q3`/`S3Q3`; toggle
  boolean check for `S3Q2`; no min/max-selection rules apply to any question.
